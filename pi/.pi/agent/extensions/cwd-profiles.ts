	...
	try {
		...
		config.directoryRules = Array.isArray(value.directoryRules)
			? value.directoryRules.flatMap((entry) => {
					...
					return [
						{ directory: resolveResource(configDir, entry.directory), profile: entry.profile },
					];
				})
			: [];
		...
		console.warn(
			`cwd-profiles: could not load ${path}: ${error instanceof Error ? error.message : error}`,
		);
		return fallback;
	}
}
...
export function selectProfile(
	...
	const originMatch =
		directoryMatch === undefined
			? config.originRules.find((rule) => rule.origin === origin)
			: undefined;
	...
	return {
		...
		instructions: uniquePaths(
			[...config.base.instructions, ...selected.instructions].map((entry) =>
				resolveResource(config.configDir, entry),
			),
		),
		skills: uniquePaths(
			[...selected.skills].map((entry) => resolveResource(config.configDir, entry)),
		),
	};
}
...
function loadInstructions(paths: string[]): Array<{ path: string; content: string }> {
	return paths.flatMap((path) => {
		...
		try {
			...
			console.warn(
				`cwd-profiles: could not read ${path}: ${error instanceof Error ? error.message : error}`,
			);
			return [];
		}
	});
...
export function filterSkillsFromPrompt(
	...
	const replacement =
		fileReadTool === undefined
			? ""
			: formatSkills(
					skills.filter((skill) => isConfiguredSkill(skill.filePath, configuredPaths)),
					fileReadTool,
				);
	return (
...
export default async function (pi: ExtensionAPI): Promise<void> {
	...
	pi.on("before_agent_start", (event) => {
		const active =
			selected ??
			selectProfile(
				loadProfileConfig(configPath),
				event.systemPromptOptions.cwd,
				originSlug(event.systemPromptOptions.cwd),
			);
