...
class GitHubPagesManager:
    ...
    def _run_gh_api(
        self, endpoint: str, method: str = "GET", data: Optional[Dict] = None, check: bool = True
    ) -> Tuple[int, Dict]:
        """
    ...
    def enable_pages(
        ...
        data = {"source": {"branch": branch, "path": path}, "build_type": build_type}
        ...
        if status == 0:
            ...
            if https_enforced:
                ...
                self._run_gh_api(endpoint, method="PUT", data={"https_enforced": True}, check=False)
...
def main():
    ...
    enable_parser.add_argument(
        "--path", default="/", choices=["/", "/docs"], help="Source path (default: /)"
    )
    enable_parser.add_argument(
        "--build-type",
        default="workflow",
    ...
    status_parser.add_argument(
        "--build-info", action="store_true", help="Show latest build information"
    )
    ...
    workflow_parser = subparsers.add_parser(
        "create-workflow", help="Create GitHub Actions workflow file"
    )
    workflow_parser.add_argument(
        "--output", help="Output path (default: .github/workflows/pages.yml)"
    )
